...
class WinMixApp {
  ...
  cacheElements() {
    const ids = ['loadingOverlay', 'toastContainer', 'resultsBody', 'listedCount', 'statTotal', 'statHome', 'statDraw', 'statAway', 'noResultsMessage', 'chartResults', 'chartBTTS', 'extendedStatsModal', 'paginationTop', 'paginationBottom', 'itemsPerPage', 'totalItems', 'pageInfoTop', 'pageInfoBottom', 'prevPageTop', 'nextPageTop', 'prevPageBottom', 'nextPageBottom'];
    ids.forEach(id => {
  ...
  async loadData() {
    ...
    try {
      const {
        data,
        error
      } = await this.supabaseClient.from('matches').select('*').order('match_time', {
        ascending: false
      });
  ...
  generateSampleData() {
    const teams = ['Ferencváros', 'Újpest', 'Debrecen', 'Paks', 'Videoton', 'Honvéd', 'Kisvárda', 'Zalaegerszeg', 'Diósgyőr', 'Gyirmót', 'Budafok', 'Soroksár', 'Barcelona', 'Real Madrid', 'Atletico Madrid', 'Sevilla', 'Valencia', 'Villarreal', 'Real Sociedad', 'Athletic Bilbao', 'Celta Vigo', 'Getafe'];
    const sampleMatches = [];
  ...
  createMatchRow(match) {
    ...
    // Result styling
    const resultClass = match.res === 'H' ? 'bg-emerald-500/10 text-emerald-300 ring-emerald-400/30' : match.res === 'A' ? 'bg-sky-500/10 text-sky-300 ring-sky-400/30' : 'bg-amber-500/10 text-amber-300 ring-amber-400/30';
  ...
  resetFilters() {
    ...
    const dropdowns = [{
      selector: '[data-dropdown="home"] [data-label]',
      text: 'Válassz hazai csapatot'
    }, {
      selector: '[data-dropdown="away"] [data-label]',
      text: 'Válassz vendég csapatot'
    }, {
      selector: '[data-dropdown="btts"] [data-label]',
      text: 'Válassz: Igen / Nem'
    }, {
      selector: '[data-dropdown="comeback"] [data-label]',
      text: 'Válassz: Igen / Nem'
    }];
    dropdowns.forEach(({
      selector,
  ...
  exportCSV() {
    ...
    const header = ['Hazai csapat', 'Vendég csapat', 'Félidő eredmény', 'Végeredmény', 'Mindkét csapat gólt szerzett', 'Fordítás'];
    const rows = [header];
    this.filteredMatches.forEach(match => {
      const row = [match.home, match.away, match.ht, match.ft, match.btts ? 'Igen' : 'Nem', match.comeback ? 'Igen' : 'Nem'];
  ...
  showExtendedStats() {
    ...
    matches.forEach(match => {
      ...
      if (match.res === 'H') homeWins++;else if (match.res === 'A') awayWins++;else draws++;
    ...
    const sortedResults = Object.entries(resultCounts).sort((a, b) => b[1] - a[1]).slice(0, 5);
